...
While the figure generally looks very similar to the one created by \citeauthor{JERMANNfinancial}, this is mostly applies to the time prior to the year 2000. After that, the figures differ slightly. The reason for this may be that the FRB updated the FFA which affected the values in this particular time period. However, the negative correlation between the two variables is still noticeable. This also holds true for the new values starting from the third quarter of 2010. 
\newpage
\vfill
\setstretch{1}
